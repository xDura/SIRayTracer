...
public:
	...
	Vector3D getPixel(unsigned int x, unsigned int y) const { return pixels[ y * width + x ]; }
	Vector3D& getPixelRef(unsigned int x, unsigned int y)	{ return pixels[ y * width + x ]; }
	Vector3D getPixelSafe(unsigned int x, unsigned int y) const {	
	...
	inline void setPixel(unsigned int x, unsigned int y, const Vector3D& c) { pixels[ y * width + x ] = c; }
	inline void setPixelSafe(unsigned int x, unsigned int y, const Vector3D& c) const { x = clamp(x, 0, width-1); y = clamp(y, 0, height-1); pixels[ y * width + x ] = c; }
	...
	//fill the image with the Vector3D C
	void fill(const Vector3D& c) { for(unsigned int pos = 0; pos < width*height; ++pos) pixels[pos] = c; }
	...
	// you can use lambda sintax:   img.forEachPixel( [](Vector3D c) { return c*2; });
	// or callback sintax:   img.forEachPixel( mycallback ); //the callback has to be Vector3D mycallback(Vector3D c) { ... }
	template <typename F>
