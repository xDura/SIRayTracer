                      ...
                      std::vector<PointLightSource>* &lightSourceList)
{
    ...
    /* ************************** */
    // (...)
	Material *green_50 = new Phong(Vector3D(0.2, 0.7, 0.3), Vector3D(0.1, 0.3, 0.1), 50.0);
	Material* red_20 = new Phong(Vector3D(0.7, 0.2, 0.2), Vector3D(0.5, 0.2, 0.2), 20.0);
	Material* other_10 = new Phong(Vector3D(0.6, 0.55, 0.55), Vector3D(0.5, 0.5, 0.5), 30.0);
    ...
    sphereTransform2 = sphereTransform2.translate(Vector3D(0.5, 1.0, 4.5));
    Shape *s2 = new Sphere (0.5, sphereTransform2, red_20);

    // Define and place a sphere
    Matrix4x4 sphereTransform3;
	sphereTransform3 = sphereTransform3.translate(Vector3D(0.5, -2.0, 5.0));
    Shape *s3 = new Sphere (1, sphereTransform3, other_10);

	Material *greyDiffuse = new Phong(Vector3D(0.8, 0.8, 0.8), Vector3D(0, 0, 0), 100);
	Shape *bottomPlan = new InfinitePlane(Vector3D(0, -3.0, 0), Vector3D(0, 1, 0), greyDiffuse);
	...
	//meshTransform = meshTransform.translate(Vector3D(0.0, 0.0, 80.0)); /** meshTransform.rotate(Utils::degreesToRadians(30), Vector3D(0.0, 1.0, 0.0));*/
	//Mesh *s4 = new Mesh(meshTransform, green_50);
	//s4->loadFromASE("box.ASE");
    ...
    // (...)
	PointLightSource pointLight = PointLightSource(Vector3D(0.3, 6.0, 3.0), Vector3D(40.0, 50.0, 50.0));
	//PointLightSource pointLight = PointLightSource(Vector3D(0.3, 5.0, 5.0), Vector3D(40.0, 50.0, 50.0));
	PointLightSource pointLight2 = PointLightSource(Vector3D(0.0, 1.5, -2.0), Vector3D(30.0, 20.0, 10.0));
    //
    // DO NOT FORGET TO STORE THE LIGHT SOURCES IN THE "lightSourceList"
    // (...)
	lightSourceList = new std::vector<PointLightSource>;
	lightSourceList->push_back(pointLight);
	lightSourceList->push_back(pointLight2);
    //
}
	...
	std::vector<Shape*>* &objectsList, std::vector<PointLightSource>* &lightSourceList)
{
	...
	Material *transmissive = new Transmissive(Vector3D(1, 1, 0.2), 1.1);
