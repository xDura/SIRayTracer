...
bool InfinitePlane::rayIntersectP(const Ray &rayWorld) const
{
    ...
    // (..)
	Vector3D normalWorld = getNormalWorld();
	float denominator = dot(rayWorld.d, normalWorld);
    ...
    //(...)
	float numerator = dot((p0World - rayWorld.o), normalWorld);
...
bool InfinitePlane::rayIntersect(const Ray &rayWorld, Intersection &its) const
{
    ...
    // (..)
	its.itsPoint = rayWorld.o + (rayWorld.d * tHit);

    // Fill the intersection details
    //(...)
	its.normal = normalWorld;
	its.shape = this;
